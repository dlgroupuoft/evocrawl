...
if [ -z "$1" ]
  then
    ...
    sleep 1m
    APPNAME=$1 DATA_FOLDER=$DATA_FOLDER INSERT_FOLDER=$INSERT_FOLDER USER_MODE='a' DEBUG_PRINT=$DBG_PRNT ENABLE_KAFKA=$KAFKA MF=$MARGIN BLIND=$BLIND REPLAY=$REPLAY MODE=$MODE INPUTS_DETECTION=$INPUTS_DETECTION pm2 start evoCrawler.sh --name aEC2_$1 #--no-autorestart
